...
def normalize_chrom_name(chrom): ##### Extracting the CHROM
...
# vcf_final['ClinVar_CLNSIG'] = vcf_final['CLIN_SIG'].apply(lambda row: "&".join(
#     [term for term in row.split("&") if term not in remove_terms]
#     ) if isinstance(row, str) and not all(term in remove_terms for term in row.split("&")) else row)
...
new_sheet_data = merged_3.copy()
# Drop duplicate rows based on 'rsID' column
new_sheet_data['EXON'].replace('', '0/0', inplace=True)
new_sheet_data['INTRON'].replace('', '0/0', inplace=True)
...
new_sheet_data = new_sheet_data.groupby('rsID').apply(filter_rows)

# Dropping duplicates based on 'rsID'
new_sheet_data = new_sheet_data.drop_duplicates('rsID')
...
# Filter rows where 'ClinVar_CLNSIG' is equal to 0
filtered_data = new_sheet_data[new_sheet_data['ClinVar_CLNSIG'] != 0]
rows_pathogenic = filtered_data[filtered_data['ClinVar_CLNSIG'].str.contains('Pathogenic|Uncertain significance|risk factor|Affects', case=False, na=False)]
rows_pathogenic_new = rows_pathogenic[~rows_pathogenic['ClinVar_CLNSIG'].str.contains('Benign', case=False, na=False)]
...
# Write each filtered dataframe to a separate sheet in the existing Excel file
df_pathogenic.to_excel(excel_writer, sheet_name='Pathogenic Variants', index=False)
df_conflicting.to_excel(excel_writer, sheet_name='Conflicting Variants', index=False)
